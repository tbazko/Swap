...
describe('Item List', () => {
  ...
  it(('should return only active items with relations'), (done) => {
    filter.byActive().addRelations().then((items) => {
      expect(items).toEqual(jasmine.any(Array));
      items.forEach((item) => {
  ...
  it('should return null if tag doesn\'t exist', (done) => {
    defaultStrategy();
